...
function parseItemLevel(html) {
  // <span>장착 아이템 레벨</span><span><small>Lv.</small>1,766<small>.66</small></span>
  const m = html.match(/<span>\s*장착 아이템 레벨\s*<\/span>\s*<span>\s*<small>Lv\.<\/small>([\d,]+)<small>([\d.]*)<\/small>\s*<\/span>/);
  if (m) return parseFloat(m[1].replace(/,/g, "") + m[2]) || 0;
  const m2 = html.match(/<span>\s*장착 아이템 레벨\s*<\/span>\s*<span>\s*<small>Lv\.<\/small>([\d,]+)\s*<\/span>/);
  if (m2) return parseFloat(m2[1].replace(/,/g, "")) || 0;
  return 0;
}

function parseCharLevel(html) {
  // <span>전투 레벨</span><span><small>Lv.</small>70</span>
  const m = html.match(/<span>\s*전투 레벨\s*<\/span>\s*<span>\s*<small>Lv\.<\/small>(\d+)\s*<\/span>/);
  ...
  if (debug) {
    try {
      ...
      return new Response(JSON.stringify({
        ...
        snippets: {
          cp: cpIdx >= 0 ? html.slice(Math.max(0, cpIdx-30), cpIdx+200) : "없음",
          il: ilIdx >= 0 ? html.slice(Math.max(0, ilIdx-30), ilIdx+200) : "없음",
          lv: lvIdx >= 0 ? html.slice(Math.max(0, lvIdx-30), lvIdx+200) : "없음",
        },
      }), { status: 200, headers });
    } catch (e) {
